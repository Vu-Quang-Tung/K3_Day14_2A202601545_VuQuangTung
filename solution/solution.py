...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        for idx, chunk in enumerate(contexts, start=1):
            ...
            if is_relevant:
                relevant_count += 1
                sum_precisions += relevant_count / idx
        if relevant_count == 0:
            return 0.0
        score = sum_precisions / relevant_count
        return max(0.0, min(1.0, score))
    ...
    def run_full_eval(
        ...
        return EvalResult(
            qa_pair=QAPair(question=question, expected_answer=expected, context=context, retrieved_contexts=contexts or []),
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            f"Answer: {answer}\n"
            f"Rubric: {rubric}\n"
            "Return JSON scores for each rubric criterion, like "
            '{"accuracy": 0.8, "clarity": 0.7}.\n'
        )
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for item in scores_batch:
            ...
            if vals:
                score_values.extend(float(v) for v in vals)
                first_scores.append(float(vals[0]))
        avg = sum(score_values) / len(score_values) if score_values else 0.0
        positional_bias = False
        if len(first_scores) >= 2:
            positional_bias = max(first_scores) - min(first_scores) > 0.1 and first_scores[0] > sum(first_scores[1:]) / (len(first_scores) - 1)
        return {
...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_context_recall": avg([float(v) for v in retrieval_recall]) if retrieval_recall else None,
            "avg_context_precision": avg([float(v) for v in retrieval_precision]) if retrieval_precision else None,
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for idx, failure in enumerate(failures, start=1):
            ...
            suggestion = suggestions[idx - 1] if idx - 1 < len(suggestions) else "Review pipeline and retriever"
            rows.append(
                f"| {failure_id} | {failure.failure_type or 'unknown'} | {root_cause} | {suggestion} | Open |"
            )
        return "\n".join(rows)
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination", 0):
            suggestions.append("Implement a stronger grounding check to block unsupported claims")
        if categories.get("irrelevant", 0) or categories.get("off_topic", 0):
            suggestions.append("Tighten intent routing and improve prompt clarity so answers stay on task")
        if categories.get("incomplete", 0):
            suggestions.append("Increase context coverage and add examples that require complete answers")
        suggestions.append("Review the full retrieval and generation pipeline for recurring failure patterns")
        while len(suggestions) < 3:
            suggestions.append("Tune prompts, retriever ranking, and answer validation for better consistency")
